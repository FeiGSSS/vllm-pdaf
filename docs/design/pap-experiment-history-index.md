...
1PA1P 的 prompt + decode 原生 APC 复用已经闭合。第二轮能否命中取决于 token LCP 和
路由到同一 PA；当前 Proxy 不实现多 PA cache-aware affinity。APC blocks 是 LRU candidate，
高压力 eviction 时允许退化为重算，但不能影响输出正确性。
...
| `PAP-20260712-MULTITURN-NORTHSTAR` | M6/M7/M10 | 临时多轮 smoke → 固定 16K/2-turn/C1 PD/PAP test bed；NIXL mailbox → same-node local-fast；HTTP EOF 计时 → last-output-token v2 | `d341f7e3e`、`3ec26b314`、`90e0b06a3` 后续 candidate | 旧 EOF-v1 PD/PAP round2 TTFT `269.01/278.48 ms`、TPOT `25.16/55.97 ms`；local-fast quick `246.59/38.60 ms` 仅为 legacy diagnostic。复审后分离 final-token 与 EOF/cleanup，并增加 provenance/lifecycle 硬门禁，v2 formal 待重建；**接受 test bed 设计，旧结果不晋升** | [北极星记录](pap-pd-multiturn-north-star-20260712.md)；`$PAP_RESULTS/20260712_{031855,032326}_*`、`$PAP_RESULTS/20260712_north_star_local_fast_quick` |
