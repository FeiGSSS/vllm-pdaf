...
> 2026-07-12 边界修正：PD、NIXL 和官方 PD proxy 必须保持未修改，正式
> PD baseline 只能通过官方配置、请求和只读 `/metrics` 驱动与观测。本文中要求
> 修改 PD/OpenAI API、增加逐请求 token 分账以及独立 OOM-admission mission 的内容
> 已被否决，不得继续按本文实施。相关实现已由 `9fde5ff6d` 撤回。
