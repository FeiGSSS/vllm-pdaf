...
def test_pa_kv_owner_reserves_next_decode_slot_from_layer_state() -> None:
    owner = PAKVOwner()
    owner.register_session(
        session_id="session-1",
        block_size=4,
    ...
    slot = owner.reserve_next_decode_slot(
        session_id="session-1",
        layer_name="model.layers.0.self_attn.attn",
    )
...
def test_pa_kv_owner_layer_reregister_preserves_materialized_coverage() -> None:
    owner = PAKVOwner()
    owner.register_session(
        session_id="session-1",
        block_size=4,
