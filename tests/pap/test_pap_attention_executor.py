...
def test_compute_offload_exec_batch_output_matches_per_request_with_prefill(
    monkeypatch,
) -> None:
    import torch

    from examples.pap.pap_attention_executor import compute_offload_exec_batch_output
    ...
    batch_registry = PAPAttentionRegistry(storage_device="cpu")
    expected_registry = PAPAttentionRegistry(storage_device="cpu")
    for registry in (batch_registry, expected_registry):
        registry.register_prefill_kv(
            PAPAttentionRegistration(
                request_id="req-a",
                conversation_id="conv",
    ...
    batch_output = compute_offload_exec_batch_output(
...
def test_compute_offload_exec_batch_output_uses_single_append_call(
    monkeypatch,
) -> None:
    import torch
    ...
    def fake_append_at_seq_len(**kwargs):
        calls.append(kwargs)
        return [(kwargs["key"], kwargs["value"])], int(kwargs["seq_len"])

    def fail_split_reserve_append(**kwargs):
        raise AssertionError("batched compute should use one append call")

    monkeypatch.setattr(
        registry,
        "append_decode_kv_at_seq_len",
        fake_append_at_seq_len,
        raising=False,
    )
    monkeypatch.setattr(registry, "reserve_decode_slot", fail_split_reserve_append)
    monkeypatch.setattr(registry, "append_decode_kv", fail_split_reserve_append)
    ...
    assert [call["request_id"] for call in calls] == ["req-a", "req-b"]
    assert [call["seq_len"] for call in calls] == [1, 1]
    torch.testing.assert_close(output, torch.tensor([[2.0, 0.0], [0.0, 3.0]]))


def test_compute_offload_exec_batch_output_uses_batch_session_lookup(
    monkeypatch,
) -> None:
    import torch
    ...
    def fail_public_session_lookup(*args, **kwargs):
        raise AssertionError("batch compute should use batch session lookup")
    ...
    monkeypatch.setattr(
        registry,
        "resolve_session_request_id",
        fail_public_session_lookup,
    )
    monkeypatch.setattr(registry, "get_session", fail_public_session_lookup)
    monkeypatch.setattr(
        registry,
        "append_decode_kv_at_seq_len",
        fake_append_at_seq_len,
...
def test_attention_registry_builds_local_paged_kv_pool(monkeypatch) -> None:
    import torch

    monkeypatch.setenv("PAP_ATTENTION_LOCAL_PAGED_CACHE", "1")
    registry = PAPAttentionRegistry(storage_device="cpu")
    layer_name = "model.layers.0.self_attn.attn"
    registry.register_prefill_kv(
        PAPAttentionRegistration(
            request_id="req-local-paged",
            conversation_id="conv",
            prefill_endpoint="http://localhost:8100",
            prefix_len=5,
            block_size=4,
    ...
    kv_cache[1, 0, :, :, :] = torch.tensor(
        [[[11.0, 11.0]], [[12.0, 12.0]], [[13.0, 13.0]], [[14.0, 14.0]]]
    )
    kv_cache[0, 1, 0, :, :] = torch.tensor([[5.0, 5.0]])
    kv_cache[1, 1, 0, :, :] = torch.tensor([[15.0, 15.0]])
    ...
    state = registry.local_paged_attention_state(
        request_id="req-local-paged",
        layer_name=layer_name,
    )
    ...
    assert state.kv_cache.shape[0] >= 2
    assert state.kv_cache.shape[1:3] == (2, 4)
    assert torch.equal(state.kv_cache[0, 0, :, :, :], kv_cache[0, 0])
    assert torch.equal(state.kv_cache[0, 1, :, :, :], kv_cache[1, 0])
    assert torch.equal(state.kv_cache[1, 0, 0, :, :], kv_cache[0, 1, 0])
    assert torch.equal(state.kv_cache[1, 1, 0, :, :], kv_cache[1, 1, 0])

    registry.append_decode_kv_at_seq_len(
        request_id="req-local-paged",
        layer_name=layer_name,
        key=torch.tensor([[[6.0, 6.0]]]),
        value=torch.tensor([[[16.0, 16.0]]]),
        seq_len=6,
    )
    state = registry.local_paged_attention_state(
        request_id="req-local-paged",
    ...
    assert torch.equal(state.kv_cache[1, 0, 1, :, :], torch.tensor([[6.0, 6.0]]))
    assert torch.equal(state.kv_cache[1, 1, 1, :, :], torch.tensor([[16.0, 16.0]]))


def test_compute_offload_exec_batch_output_uses_local_paged_flash_attention(
    monkeypatch,
) -> None:
    import torch
    ...
    monkeypatch.setenv("PAP_ATTENTION_LOCAL_PAGED_CACHE", "1")
    monkeypatch.setenv("PAP_OFFLOAD_EXEC_USE_PAGED_FLASH_ATTN", "1")
    monkeypatch.setenv("PAP_OFFLOAD_EXEC_NUM_HEADS", "1")
    monkeypatch.setenv("PAP_OFFLOAD_EXEC_NUM_KV_HEADS", "1")
    monkeypatch.setenv("PAP_OFFLOAD_EXEC_HEAD_DIM", "2")
    registry = PAPAttentionRegistry(storage_device="cpu")
    layer_name = "model.layers.0.self_attn.attn"
    for request_id, base in (("req-a", 1.0), ("req-b", 10.0)):
        registry.register_prefill_kv(
            PAPAttentionRegistration(
                request_id=request_id,
                conversation_id="conv",
        ...
        kv_cache = torch.zeros((2, 1, 4, 1, 2))
        kv_cache[0, 0, :2, :, :] = torch.tensor(
            [[[base, base]], [[base + 1.0, base + 1.0]]]
        )
        kv_cache[1, 0, :2, :, :] = torch.tensor(
            [[[base + 20.0, base + 20.0]], [[base + 21.0, base + 21.0]]]
        )
        registry.import_prefill_paged_kv(
            request_id=request_id,
            layer_name=layer_name,
    ...
    def fake_paged_flash_batch(**kwargs):
        calls.append(kwargs)
        return torch.tensor([[2.0, 0.0], [0.0, 3.0]])

    def fail_sdpa(*args, **kwargs):
        raise AssertionError("paged path should not fall back to padded SDPA")

    monkeypatch.setattr(
        executor_module,
        "_compute_offload_exec_paged_flash_batch",
        fake_paged_flash_batch,
    )
    ...
    output = compute_offload_exec_batch_output(
        ...
        qkv_batch=torch.tensor(
            [
                [1.0, 0.0, 3.0, 3.0, 23.0, 23.0],
                [0.0, 1.0, 13.0, 13.0, 33.0, 33.0],
            ]
        ),
    )

    assert len(calls) == 1
    assert calls[0]["query_batch"].shape == (2, 1, 2)
    assert calls[0]["states"][0].seq_len == 3
    assert calls[0]["states"][1].seq_len == 3
    assert calls[0]["states"][0].pool is calls[0]["states"][1].pool
    assert calls[0]["states"][0].block_ids == (0,)
    assert calls[0]["states"][1].block_ids == (1,)
    torch.testing.assert_close(output, torch.tensor([[2.0, 0.0], [0.0, 3.0]]))


def test_local_paged_flash_batch_skips_segment_decode_buffer(
    monkeypatch,
) -> None:
    import torch
    ...
    def fail_segment_append(**kwargs):
        raise AssertionError(
            "paged FlashAttention batch should not build segment decode KV"
        )
    ...
    assert len(calls) == 1
    assert [state.seq_len for state in calls[0]["states"]] == [3, 3]
    for request_id in ("req-a", "req-b"):
        session_id = registry.resolve_session_request_id(request_id)
        assert session_id is not None
        assert layer_name not in registry._decode_kv[session_id]
    torch.testing.assert_close(output, torch.tensor([[2.0, 0.0], [0.0, 3.0]]))


def test_compute_offload_exec_batch_output_uses_batched_qkv_append(
    monkeypatch,
) -> None:
    import torch
    ...
    states = [
        executor_module.PAPLocalPagedAttentionState(
            pool=executor_module.PAPLocalPagedKVPool(
                layer_name=layer_name,
                kv_cache=torch.empty((4, 2, 4, 1, 2)),
                block_size=4,
                num_kv_heads=1,
                head_dim=2,
            ),
    ...
    def fake_batched_append(**kwargs):
        append_calls.append(kwargs)
        return states

    def fail_legacy_append(**kwargs):
        raise AssertionError("paged fast path should append batched key/value")
    ...
    output = compute_offload_exec_batch_output(
        ...
        qkv_batch=torch.tensor(
            [
                [1.0, 0.0, 3.0, 4.0, 30.0, 40.0],
                [0.0, 1.0, 13.0, 14.0, 130.0, 140.0],
            ]
        ),
    ...
    assert append_calls[0]["session_request_ids"] == ("req-a", "req-b")
    assert append_calls[0]["layer_name"] == layer_name
    assert append_calls[0]["seq_lens"] == (3, 3)
    assert append_calls[0]["trace_stats"] is not None
    torch.testing.assert_close(
        append_calls[0]["key_batch"],
        torch.tensor([[[3.0, 4.0]], [[13.0, 14.0]]]),
    )
    torch.testing.assert_close(
        append_calls[0]["value_batch"],
        torch.tensor([[[30.0, 40.0]], [[130.0, 140.0]]]),
    )
    torch.testing.assert_close(output, torch.tensor([[2.0, 0.0], [0.0, 3.0]]))


def test_local_paged_copy_source_reuses_matching_strided_view() -> None:
    import torch

    from examples.pap.pap_attention_executor import _local_paged_copy_source

    packed = torch.arange(24, dtype=torch.float32).reshape(4, 6)
    strided_view = packed[:, 2:4].view(4, 1, 2)

    assert not strided_view.is_contiguous()
    source = _local_paged_copy_source(
        strided_view,
        device=strided_view.device,
        dtype=strided_view.dtype,
    )

    assert source.data_ptr() == strided_view.data_ptr()
    assert source.stride() == strided_view.stride()


def test_local_paged_slot_mapping_tensor_uses_flat_cache_slots() -> None:
    import torch

    from examples.pap.pap_attention_executor import _local_paged_slot_mapping_tensor

    slot_mapping = _local_paged_slot_mapping_tensor(
        local_blocks=[3, 5, 7],
        block_offsets=[2, 0, 15],
        block_size=16,
    ...
    torch.testing.assert_close(
        slot_mapping,
        torch.tensor([50, 80, 127], dtype=torch.long),
    )


def test_local_paged_tensor_batch_append_uses_native_cache_append(
    monkeypatch,
) -> None:
    import torch
    ...
    monkeypatch.setenv("PAP_ATTENTION_LOCAL_PAGED_CACHE", "1")
    monkeypatch.setenv("PAP_ATTENTION_LOCAL_PAGED_NATIVE_CACHE_APPEND", "1")
    registry = PAPAttentionRegistry(storage_device="cpu")
    layer_name = "model.layers.0.self_attn.attn"
    for request_id, base in (("req-a", 1.0), ("req-b", 10.0)):
        registry.register_prefill_kv(
    ...
    def fake_native_cache_append(**kwargs):
        native_calls.append(kwargs)
        return True

    monkeypatch.setattr(
        executor_module,
        "_try_local_paged_native_cache_append",
        fake_native_cache_append,
    )

    states = registry.append_decode_kv_tensor_batch_for_local_paged_attention(
        session_request_ids=("req-a", "req-b"),
        layer_name=layer_name,
        key_batch=torch.tensor([[[3.0, 4.0]], [[13.0, 14.0]]]),
        value_batch=torch.tensor([[[30.0, 40.0]], [[130.0, 140.0]]]),
        seq_lens=(3, 3),
    )
    ...
    torch.testing.assert_close(
        native_calls[0]["slot_mapping"],
        torch.tensor([2, 6], dtype=torch.long),
    )
    torch.testing.assert_close(
        native_calls[0]["key_batch"],
        torch.tensor([[[3.0, 4.0]], [[13.0, 14.0]]]),
    )
    torch.testing.assert_close(
        native_calls[0]["value_batch"],
        torch.tensor([[[30.0, 40.0]], [[130.0, 140.0]]]),
    )


def test_local_paged_tensor_batch_append_uses_resolved_session_ids(
    monkeypatch,
) -> None:
    import torch
    ...
    def fail_resolve(*args, **kwargs):
        raise AssertionError("batch append should use resolved session ids")

    monkeypatch.setattr(registry, "_resolve_session_request_id_locked", fail_resolve)
    ...
    assert states is not None
    assert [state.seq_len for state in states] == [3, 3]


def test_local_paged_tensor_batch_append_skips_prefill_wait_for_existing_state(
    monkeypatch,
) -> None:
    import torch
    ...
    def fail_wait(*args, **kwargs):
        raise AssertionError("existing local paged state should skip prefill wait")

    monkeypatch.setattr(registry, "_wait_for_prefill_layer_locked", fail_wait)
...
def test_local_paged_tensor_batch_append_writes_from_strided_qkv_views(
    monkeypatch,
) -> None:
    import torch
    ...
    _, key_flat, value_flat = qkv_batch.split((2, 2, 2), dim=-1)
    key_batch = key_flat.view(2, 1, 2)
    value_batch = value_flat.view(2, 1, 2)

    assert not key_batch.is_contiguous()
    assert not value_batch.is_contiguous()
    ...
    assert [state.seq_len for state in states] == [3, 3]
    torch.testing.assert_close(
        states[0].kv_cache[states[0].block_ids[0], 0, 2, :, :],
        torch.tensor([[3.0, 4.0]]),
    )
    torch.testing.assert_close(
        states[0].kv_cache[states[0].block_ids[0], 1, 2, :, :],
        torch.tensor([[30.0, 40.0]]),
    )
    torch.testing.assert_close(
        states[1].kv_cache[states[1].block_ids[0], 0, 2, :, :],
        torch.tensor([[13.0, 14.0]]),
    )
    torch.testing.assert_close(
        states[1].kv_cache[states[1].block_ids[0], 1, 2, :, :],
        torch.tensor([[130.0, 140.0]]),
    )


def test_local_paged_batch_append_writes_without_per_request_append(
    monkeypatch,
) -> None:
    import torch

    from examples.pap.pap_attention_executor import PAPLocalPagedDecodeAppend
    ...
    def fail_per_request_append(**kwargs):
        raise AssertionError("local paged batch append should write in batch")

    monkeypatch.setattr(
        registry,
        "_append_local_paged_decode_locked",
        fail_per_request_append,
    )

    states = registry.append_decode_kv_batch_for_local_paged_attention(
        entries=[
            ...
            PAPLocalPagedDecodeAppend(
                request_id="req-b",
                layer_name=layer_name,
                key=torch.tensor([[[13.0, 14.0]]]),
                value=torch.tensor([[[130.0, 140.0]]]),
                seq_len=3,
            ),
        ]
    )
...
def test_build_paged_flash_metadata_pads_block_table() -> None:
    import torch

    from examples.pap.pap_attention_executor import (
        PAPLocalPagedAttentionState,
        PAPLocalPagedKVPool,
        build_paged_flash_metadata,
    )
    ...
    states = [
        PAPLocalPagedAttentionState(pool=pool, block_ids=(0, 1), seq_len=6),
        PAPLocalPagedAttentionState(pool=pool, block_ids=(2,), seq_len=3),
    ]

    metadata = build_paged_flash_metadata(states=states, device=torch.device("cpu"))

    assert metadata.max_seq_len == 6
    torch.testing.assert_close(
...
def test_build_paged_flash_metadata_reuses_pool_scratch() -> None:
    import torch
    ...
    first = build_paged_flash_metadata(
        states=[
            PAPLocalPagedAttentionState(pool=pool, block_ids=(0, 1), seq_len=6),
            PAPLocalPagedAttentionState(pool=pool, block_ids=(2,), seq_len=3),
        ],
    ...
    first_block_ptr = first.block_table.data_ptr()
    first_seq_lens_ptr = first.seq_lens.data_ptr()
    first_cu_ptr = first.cu_seqlens_q.data_ptr()

    second = build_paged_flash_metadata(
        states=[
            PAPLocalPagedAttentionState(pool=pool, block_ids=(0, 1), seq_len=7),
            PAPLocalPagedAttentionState(pool=pool, block_ids=(2, 3), seq_len=8),
        ],
        device=torch.device("cpu"),
    )

    assert second.block_table.data_ptr() == first_block_ptr
    assert second.seq_lens.data_ptr() == first_seq_lens_ptr
    assert second.cu_seqlens_q.data_ptr() == first_cu_ptr
    assert second.max_seq_len == 8
    torch.testing.assert_close(
        second.block_table,
...
def test_device_matches_request_accepts_unspecified_cuda_index() -> None:
    import torch

    from examples.pap.pap_attention_executor import _device_matches_request

    assert _device_matches_request(torch.device("cuda:0"), torch.device("cuda"))
    assert _device_matches_request(torch.device("cuda:0"), torch.device("cuda:0"))
    assert not _device_matches_request(torch.device("cuda:1"), torch.device("cuda:0"))
    assert not _device_matches_request(torch.device("cpu"), torch.device("cuda"))


def test_cuda_metadata_scratch_is_explicit_opt_in() -> None:
    text = (ROOT / "examples" / "pap" / "pap_attention_executor.py").read_text()

    assert "PAP_ATTENTION_PAGED_METADATA_SCRATCH_CUDA" in text
    assert 'device.type != "cuda"' in text
