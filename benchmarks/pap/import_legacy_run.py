...
def _gate(
    ...
    return {
        "status": status,
        "detail": detail,
        "evidence": [_path_reference(path, root_id, root) for path in paths],
    }
...
def import_legacy_run(
    ...
    *,
    ...
    artifacts = [_make_artifact(aggregate_path, None, artifact_root_id, artifact_root)]
    ...
    result_paths = [payload["directory"] / "result.json" for payload in rep_payloads]
    correctness_paths = [
        payload["directory"] / "correctness_audit.env" for payload in rep_payloads
    ]
    join_paths = [
        payload["directory"] / "decode_token_join_audit.env" for payload in rep_payloads
    ]
    ...
    routing_paths = [
        payload["directory"] / "routing_audit.json" for payload in rep_payloads
    ]
    drain_paths = [
        payload["directory"] / "session_drain.env" for payload in rep_payloads
    ]
    mps_paths = [
        payload["directory"] / "mps_static_audit_pa_0.env" for payload in rep_payloads
    ]
    attention_paths = [
    ...
    if profile_id == "p17_1pa1p" and expected_mps_layout not in known_p17_layouts:
        mps_status = "failed"
    for payload in rep_payloads:
        rep_mps = payload["mps"]
        if (
    ...
    audits = {
        ...
        "lease": _gate(
            ("passed" if correctness_status == drain_status == "passed" else "failed"),
    ...
    unified_kv_fallback: bool | str = True if profile_id == "p17_1pa1p" else "missing"
    ...
    return {
        ...
        "provenance": {
            ...
            "tracked_worktree_patch_sha256": _uniform(worktree_patch_hashes),
