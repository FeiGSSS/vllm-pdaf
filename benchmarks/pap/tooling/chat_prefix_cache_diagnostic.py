...
def _run_chat_completion(
    ...
    *,
    ...
    if (
        result["prefill_cached_tokens"] + result["prefill_computed_tokens"]
        != prompt_tokens
    ):
...
def _validate_fixed_length(
    ...
    *,
    ...
    if response["finish_reason"] != "length":
        raise RuntimeError(
            f"chat completion did not finish by length: {response['finish_reason']}"
        )
...
def run_audit(args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        ...
        "first_assistant_text_digest": _text_digest(first["assistant_content"]),
        "second_assistant_text_digest": _text_digest(warm["assistant_content"]),
