...
def validate_trace(
    ...
    *,
    ...
    if payload.get("role") != contract["role"]:
        raise ValueError(
            f"deferred trace role mismatch: {payload.get('role')} != {contract['role']}"
        )
    ...
    layer_counts = {
        str(name): _span_count(spans, str(name)) for name in contract["layer_spans"]
    }
    ...
    if layers <= 0 or layer_calls % layers != 0:
        raise ValueError(f"layer-call count {layer_calls} is not divisible by {layers}")
    ...
    peer_counts = {
        str(name): _span_count(spans, str(name)) for name in contract["peer_spans"]
    }
    ...
    if reference_peer_batches is not None:
        ...
        if peer_batches != reference:
            raise ValueError(
                f"Attention peer-batch mismatch: {peer_batches} != {reference}"
            )
    return {
