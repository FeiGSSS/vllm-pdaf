...
def _load_profiles(directory: Path) -> tuple[dict[str, dict[str, Any]], list[str]]:
    ...
    for path in sorted(directory.rglob("*.toml")):
        ...
        if path.stem != profile_id:
            errors.append(f"{path}: filename must match profile_id {profile_id}")
...
def _validate_archived_p17_run_contract(
    ...
    expected = {
        ...
        "workload.model.tensor_parallel_size": profile["model"]["tensor_parallel_size"],
        ...
        "placement.attention_devices": profile["placement"]["attention_devices"],
        "placement.projection_devices": profile["placement"]["projection_devices"],
        ...
        "fingerprints.profile": profile["baseline_fingerprints"]["workload_profile"],
        "fingerprints.request_shape": profile["baseline_fingerprints"]["request_shape"],
    }
    errors = [
        f"{run['run_id']}: P17 {key} must be {wanted!r}, got {_nested(run, key)!r}"
        for key, wanted in expected.items()
    ...
    if mps_layout is None:
        errors.append(
            f"{run['run_id']}: P17 mps.profile_id is not recognized: {mps_profile_id!r}"
        )
    else:
    ...
    expected_completed = (
        profile["audit"]["completed_requests_per_repetition"] * profile["repetitions"]
    )
...
def _validate_profiles(profiles: dict[str, dict[str, Any]]) -> list[str]:
    ...
    canonical_release_profiles = [
        ...
        if profile.get("status") == "canonical" and profile.get("release_gate") is True
    ]
...
def _validate_run_semantics(
    ...
    artifact_paths = {_artifact_key(item["path"]) for item in run["artifacts"]}
    ...
    if run["profile_id"] == "p17_1pa1p" and set(run["audits"]) != REQUIRED_AUDITS:
    ...
    for gate_name, gate in run["audits"].items():
        for reference in gate["evidence"]:
            if _artifact_key(reference) not in artifact_paths:
                errors.append(f"{run_id}: {gate_name} evidence is not an artifact")
    ...
    if run["evidence"] == "formal-clean":
        ...
        for patch_field in (
            ...
            if provenance[patch_field] != EMPTY_SHA256:
                errors.append(f"{run_id}: formal-clean {patch_field} must be empty")
        if repetitions["count"] < 3:
            errors.append(f"{run_id}: formal-clean evidence requires 3 repetitions")
        failed_gates = [
            name for name, gate in run["audits"].items() if gate["status"] != "passed"
        ]
        if failed_gates:
...
def _validate_experiment_semantics(
    ...
    for run_id in experiment["run_ids"]:
        ...
        if run is None:
            ...
            errors.append(
                f"{experiment_id}: run {run_id} belongs to {run['experiment_id']}"
            )
    ...
    for arm_name in ("baseline", "treatment"):
        ...
        if run is None:
            continue
        observations = {
            (metric["name"], metric["scope"], metric["statistic"]): metric["value"]
...
def validate_registry(
    *,
    ...
    experiment_schema = _read_json(SCHEMA_DIR / "experiment_record.schema.json")
