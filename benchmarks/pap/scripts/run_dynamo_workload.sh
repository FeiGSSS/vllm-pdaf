...
RUN_ROOT="${DYNAMO_RUN_ROOT:-${ROOT_DIR}/benchmarks/pap/experiments/_staging/dynamo/${RUN_ID}}"
...
AIPERF_INPUT_FILE="${DYNAMO_AIPERF_INPUT_FILE:-${DEFAULT_DATASET}}"
AIPERF_OUTPUT_DIR="${DYNAMO_AIPERF_OUTPUT_DIR:-${RUN_ROOT}/aiperf}"
AIPERF_SESSIONS="${DYNAMO_AIPERF_SESSIONS:-${PAP_AGENTIC_BASELINE_SESSIONS}}"
AIPERF_CONCURRENCY="${DYNAMO_AIPERF_CONCURRENCY:-${PAP_AGENTIC_BASELINE_CONCURRENCY}}"
AIPERF_TIMING_MODE="${DYNAMO_AIPERF_TIMING_MODE:-request_rate}"
AIPERF_REQUEST_RATE="${DYNAMO_AIPERF_REQUEST_RATE:-${PAP_AGENTIC_BASELINE_REQUEST_RATE}}"
AIPERF_ARRIVAL_PATTERN="${DYNAMO_AIPERF_ARRIVAL_PATTERN:-${PAP_AGENTIC_BASELINE_ARRIVAL_PATTERN}}"
...
AIPERF_WARMUP_DURATION_SECONDS="${DYNAMO_AIPERF_WARMUP_DURATION_SECONDS:-${PAP_AGENTIC_BASELINE_WARMUP_SECONDS}}"
AIPERF_BENCHMARK_DURATION_SECONDS="${DYNAMO_AIPERF_BENCHMARK_DURATION_SECONDS:-${PAP_AGENTIC_BASELINE_DURATION_SECONDS}}"
AIPERF_BENCHMARK_GRACE_PERIOD_SECONDS="${DYNAMO_AIPERF_BENCHMARK_GRACE_PERIOD_SECONDS:-${PAP_AGENTIC_BASELINE_GRACE_SECONDS}}"
...
"${DYNAMO_PYTHON}" -P -m pip freeze > "${RUN_ROOT}/python_packages.txt"
{
  ...
  printf 'AIPERF_INPUT_SHA256=%q\n' \
    "$(sha256sum "${AIPERF_INPUT_FILE}" | awk '{print $1}')"
  printf 'GIT_COMMIT=%q\n' "$(git rev-parse HEAD)"
