...
AIPERF_SESSIONS="${DYNAMO_AIPERF_SESSIONS:-128}"
AIPERF_CONCURRENCY="${DYNAMO_AIPERF_CONCURRENCY:-64}"
AIPERF_TIMING_MODE="${DYNAMO_AIPERF_TIMING_MODE:-request_rate}"
AIPERF_REQUEST_RATE="${DYNAMO_AIPERF_REQUEST_RATE:-2}"
...
AIPERF_EXPECTED_REQUESTS="${DYNAMO_AIPERF_EXPECTED_REQUESTS:-1630}"
...
case "${ARCHITECTURE}" in
  ...
  6p2d)
    ...
    ;;
  *) die "DYNAMO_ARCHITECTURE must be dp8 or 6p2d" ;;
esac
GPU_COUNT=$((AGG_COUNT + PREFILL_COUNT + DECODE_COUNT))
if (( AGG_COUNT > 0 )); then
  ROUTABLE_COUNT="${AGG_COUNT}"
else
  ROUTABLE_COUNT="${DECODE_COUNT}"
fi
case "${ROUTER_MODE}" in
...
printf 'STATUS=passed\nMATCH_COUNT=0\n' \
  > "${RUN_ROOT}/correctness_audit.env"
if (( SMOKE_ONLY == 0 )) && [[ "${ARCHITECTURE}" == "6p2d" ]]; then
  "${PAP_PYTHON}" \
