...
for required in "${PYTHON_BIN}" "${VLLM_BIN}" "${AIPERF_BIN}" \
  "${AIPERF_RUNNER}" "${CUDA_GRAPH_AUDITOR}" "${MODEL_PATH}" \
  "${INPUT_FILE}"; do
  [[ -e "${required}" ]] || die "required path is missing: ${required}"
done

DP_CUDAGRAPH_CAPTURE_SIZES="${DP_LOAD_CUDAGRAPH_CAPTURE_SIZES:-1,2,4,8,12,16,20,24,28,32,64,128}"
[[ "${DP_CUDAGRAPH_CAPTURE_SIZES}" \
  =~ ^[1-9][0-9]*(,[1-9][0-9]*)*$ ]] \
  || die "DP CUDA Graph capture sizes must be positive integer CSV"
DP_COMPILATION_CONFIG="{\"mode\":\"VLLM_COMPILE\",\"cudagraph_mode\":\"PIECEWISE\",\"cudagraph_capture_sizes\":[${DP_CUDAGRAPH_CAPTURE_SIZES}]}"
EXECUTION_ARGS=(
  --compilation-config "${DP_COMPILATION_CONFIG}"
)
TOOL_ARGS=()
...
DP_VLLM_GRAPH_LOGS=()
for (( rank=0; rank<DP_SIZE; rank++ )); do
  DP_VLLM_GRAPH_LOGS+=("${LOG_ROOT}/dp_${rank}.log")
done
"${CUDA_GRAPH_AUDITOR}" "${RUN_ROOT}/vllm_cuda_graph_audit.env" \
  PIECEWISE "${DP_VLLM_GRAPH_LOGS[@]}"
