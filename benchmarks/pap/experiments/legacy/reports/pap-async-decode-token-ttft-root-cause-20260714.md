...
| 实验 | 目录 | 用途 |
| --- | --- | --- |
| 两侧无 barrier | `benchmarks/pap/experiments/legacy/runs/20260714_prefill_scope_ab_none_c4` | 当前 async 行为 |
| 仅 Projection barrier | `benchmarks/pap/experiments/legacy/runs/20260714_prefill_scope_ab_projection_only_c4` | Projection 侧因果干预 |
| 仅 Prefill barrier | `benchmarks/pap/experiments/legacy/runs/20260714_prefill_scope_ab_prefill_only_c4` | 排除 Prefill 侧直接作用 |
| 两侧 barrier | `benchmarks/pap/experiments/legacy/runs/20260714_prefill_scope_ab_both_c4` | 复现旧全局 sync-only 语义 |
| 旧 Projection single batch | `benchmarks/pap/experiments/legacy/runs/20260714_async_token_projection_single_batch_c4` | async queue 深度诊断 |
| 当前 clean async/static | `benchmarks/pap/experiments/legacy/runs/20260714_cbcbfabcb_async_static_clean_c4` | trace-off 参考 |
| R1 gate、无 Projection barrier | `benchmarks/pap/experiments/legacy/runs/20260714_strict_isolation_projection_gate_async_c4` | 完全排除 Decode 的 Prefill 窗口 |
| R1 gate、有 Projection barrier | `benchmarks/pap/experiments/legacy/runs/20260714_strict_isolation_projection_gate_sync_c4` | 验证 barrier 无直接 Prefill 作用 |
| Decode commit gate | `benchmarks/pap/experiments/legacy/runs/20260714_strict_isolation_commit_gate_async_c4` | 只切断 Decode 到 Prefill 的控制反馈 |
| 正常 async 严格 control | `benchmarks/pap/experiments/legacy/runs/20260714_strict_isolation_normal_async_c4` | commit gate 的同版本 trace-off 对照 |
| pure-Prefill Torch trace | `benchmarks/pap/experiments/legacy/runs/20260714_prefill_torch_profile_no_decode_c4` | kernel count/busy 和 host gap 基准 |
| 正常 Decode Torch trace | `benchmarks/pap/experiments/legacy/runs/20260714_prefill_torch_profile_normal_async_c4` | 证明 `2.81 s` 增量为 host-unsubmitted gap |
| 同步 IPC profile | `benchmarks/pap/experiments/legacy/runs/20260714_prefill_ipc_profile_sync_c4` | 将 host gap 对齐到逐层 TCP/registry 等待 |
| 异步 IPC profile（缩锁前） | `benchmarks/pap/experiments/legacy/runs/20260714_prefill_ipc_profile_async_fixed_c4` | 分离 registry queue wait 与 IPC open/install |
| 缩锁、同步 import | `benchmarks/pap/experiments/legacy/runs/20260714_registry_lock_narrow_sync_c4` | 单独验证主修复的因果收益 |
| 缩锁、异步但缺 readiness | `benchmarks/pap/experiments/legacy/runs/20260714_registry_lock_narrow_async_c4` | 输出分叉反例，禁止作为正确候选 |
| 缩锁、安全异步单次 | `benchmarks/pap/experiments/legacy/runs/20260714_registry_lock_narrow_async_ready_c4` | 完整-prefix readiness 正确性验证 |
| 缩锁、安全异步三次 | `benchmarks/pap/experiments/legacy/runs/20260714_registry_lock_narrow_async_ready_c4_reps3` | dirty-development 可重复性；三份 raw 通过，聚合器按设计拒绝 dirty formal |
| test bed 默认接线 C1 | `benchmarks/pap/experiments/legacy/runs/20260714_registry_lock_default_async_fingerprint_c1` | 未显式传 async/profile；验证默认值、implementation fingerprint、五轮 APC 和严格 audit |
