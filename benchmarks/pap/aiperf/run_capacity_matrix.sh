...
PAP_PREFILL_MAX_INFLIGHT_PER_PA="${PAP_CAPACITY_PAP_PREFILL_MAX_INFLIGHT_PER_PA:-0}"
...
PAP_NIXL_CONNECTOR_LEASE_SECONDS="$(
  printf '%s' "${PAP_CAPACITY_PAP_NIXL_CONNECTOR_LEASE_SECONDS:-1}"
)"
...
for value in "${PAP_PREFILL_CHUNKS}" "${PAP_ATTENTION_CHUNKS}" \
  ...
  "${PAP_NIXL_CONNECTOR_LEASE_SECONDS}"; do
  [[ "${value}" =~ ^[1-9][0-9]*$ ]] \
...
[[ "${PAP_PREFILL_MAX_INFLIGHT_PER_PA}" =~ ^[0-9]+$ ]] \
  || die "PAP_CAPACITY_PAP_PREFILL_MAX_INFLIGHT_PER_PA must be non-negative"
...

{
  ...
  printf 'PAP_PREFILL_MAX_INFLIGHT_PER_PA=%q\n' \
    "${PAP_PREFILL_MAX_INFLIGHT_PER_PA}"
...
run_pap_architecture() {
  ...
  env \
    ...
    PAP_PREFILL_MAX_INFLIGHT_PER_PA="${PAP_PREFILL_MAX_INFLIGHT_PER_PA}" \
